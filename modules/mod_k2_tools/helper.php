...
class modK2ToolsHelper {
	...
	function treerecurse(&$params, $id = 0, $level = 0, $begin = false) {
		...
		if ($level < intval($end_level) || is_null($end_level)) {
			$output .= '<ul class="menu demo-list medmarginbottom">';
			foreach ($rows as $row) {
...
#					$output .= '<li'.$active.'><a href="'.urldecode(JRoute::_(K2HelperRoute::getCategoryRoute($row->id.':'.urlencode($row->alias)))).'"><span class="catTitle">'.$row->name.'</span><span class="catCounter">'.$row->numOfItems.'</span></a>';
					...
					$output .= '<li'.$active.'><span class="item subtext"><span>'.$row->name.'<em><a href="'.urldecode(JRoute::_(K2HelperRoute::getCategoryRoute($row->id.':'.urlencode($row->alias)))).'">'.strip_tags(substr($row->description,0,60)).' ...</a></em></span></span></li>';
