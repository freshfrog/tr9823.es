 ...
 * @package    RocketTheme
 * @subpackage roknewspager.fields
 */
class JFormFieldOverlayList extends JFormField
{
    /**
     * Element name
     *
    ...
    function getInput()
    {
        ...
        $filter  = '\.png$';
        $exclude = $this->element->attributes('exclude');
        ...
        $theme                   = $this->form->getValue("theme");
        ...
        $options = array();
        if (!$this->element->attributes('hide_none')) {
        ...
        return JHTML::_('select.genericlist', $options, $this->name, 'class="inputbox"', 'value', 'text', $this->value, "param{$this->name}");
    }
