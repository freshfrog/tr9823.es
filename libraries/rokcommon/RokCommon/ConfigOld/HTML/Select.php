...
abstract class RokCommon_Config_HTML_Select
{
	...
	*/
	public static function booleanlist(
		...
		$arr = array(
			RokCommon_Config_HTML_Select::Option( '0', rc__($no)),
			RokCommon_Config_HTML_Select::Option( '1', rc__($yes))
		);
		return RokCommon_Config_HTML_Select::radiolist( $arr, $name, $attribs, 'value', 'text', (int) $selected, $id);
	}
	...
	public static function genericlist(
		...
		// Set default options
		$options = array_merge(
			RokCommon_Config_HTML_Select::$formatOptions,
			array(
				'format.depth' => 0,
				'id' => false,
			)
		...
		foreach ($arr as $elementKey => &$element)
		{
			...
			if ($options['groups'] && $key == '<OPTGROUP>') {
				$html .= $baseIndent . '<optgroup label="'
					. ($options['list.translate'] ? rc__($text) : $text)
					. '">' . $options['format.eol'];
				$baseIndent = str_repeat($options['format.indent'], ++$options['format.depth']);
