...
class JFormFieldPerItemPicker extends JFormField
{
	...
	function getInput(){
		...
		$html[] = '		<input data-peritempicker-display="true" data-original-title="'.$this->value.'" type="text" value="'.$this->value.'" '.$class.$placeholder.' />';
	...
	public function _getDropdown(){
		...
		// original select
		$output[] = '		<select'.((count($options) == 1) ? ' style="display: none;"' : '').'>';
		foreach($options as $option){
			$attributes = $option['attributes'];
			$divider = (isset($attributes['data-divider']))? $attributes['data-divider'] : "";
			$dataValue = (isset($attributes['data-value']))? $attributes['data-value'] : '';
