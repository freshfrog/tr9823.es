...
<form action="<?php echo JRoute::_('index.php?option=com_rokcandy'); ?>" method="post" name="adminForm" id="adminForm">
	<fieldset id="filter-bar">
		<div class="filter-search fltlft">
			...
			<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" title="<?php echo JText::_('ROKCANDY_SEARCH_IN_NAME'); ?>" />
    ...
    <?php $i=0; $i++; $n=0;?>
        <?php foreach ($this->overrides as $macro=>$html) : ?>
            <tr class="<?php echo $i % 2; ?>">
                ...
                <td class="order"><input type="text" name="order[]" size="5" value="<?php echo 'n/a';?>" disabled="disabled" class="text-area-order" /></td>
