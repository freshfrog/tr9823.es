 ...
 * @since          1.6
 */
class RokSprocketViewModule extends JView
{
	public $form;
	public $item;
	public $state;
	...
	protected function compileLess()
	{
		...
		RokCommon_Header::addStyle(JURI::base(true) . '/components/com_roksprocket/assets/styles/roksprocket.css?nocache=1.5');
	}
	...
	protected function compileJS()
	{
		...
		$site_path  = JPATH_ROOT . DS . 'components' . DS . 'com_roksprocket';
		...
		if (defined('DEV') && DEV) {
			...
			$admin_assets  = $admin_path . DS . 'assets' . DS . 'js' . DS;
			$app           = $admin_path . DS . 'assets' . DS . 'application' . DS;
			$filters       = $site_path . DS . 'fields' . DS . 'filters' . DS . 'js' . DS;
			$imagepicker   = $site_path . DS . 'fields' . DS . 'imagepicker' . DS . 'js' . DS;
			$peritempicker = $site_path . DS . 'fields' . DS . 'peritempicker' . DS . 'js' . DS;
		...
		RokCommon_Header::addScript(JURI::base(true) . '/components/com_roksprocket/assets/js/roksprocket.js?nocache=1.5');
